...
QList<Diff> diff_match_patch::diff_compute(QString text1, QString text2,
...

  {
    ...
    if (i != -1) {
      ...
      const Operation op = (text1.length() > text2.length()) ? DEL : INSERT;
  ...
  while (thisDiff != NULL) {
    if (thisDiff->operation == EQUAL) {
      ...
      if (!lastequality.isNull()
        ...
        pointer.setValue(Diff(DEL, lastequality));
      ...
      case EQUAL:
        if (count_delete + count_insert > 1) {
          ...
          if (!text_delete.isEmpty()) {
            pointer.insert(Diff(DEL, text_delete));
          }
...
QString diff_match_patch::diff_toString(const QList<Diff> &diffs) {
  QString text;
  foreach(Diff aDiff, diffs) {
    switch (aDiff.operation) {
      case INSERT: {
        
		  text += QString("+") + aDiff.text + QString("\n");
        break;
      }
      case DEL:
        text += QString("-") + aDiff.text
            + QString("\n");
        break;
      case EQUAL:
        text += QString("=") + QString::number(aDiff.text.length())
    ...
    while (!bigpatch.diffs.isEmpty()) {
      ...
      while (!bigpatch.diffs.isEmpty()
        ...
        if (diff_type == INSERT) {
          ...
          empty = false;
        } else if (diff_type == DEL && patch.diffs.size() == 1
...
QList<Patch> diff_match_patch::patch_fromText(const QString &textline) {
  ...
  while (!text.isEmpty()) {
    ...
    while (!text.isEmpty()) {
      ...
      if (sign == '-') {
        // Deletion.
        patch.diffs.append(Diff(DEL, line));
      } else if (sign == '+') {
        // Insertion.
